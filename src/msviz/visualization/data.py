...
def load_runtime_data(csv_path: str = "data/anonymized_runtime_data.csv") -> pd.DataFrame:
    path = Path(csv_path)
    if not path.is_absolute() and not path.exists():
        ...
        candidate_paths = [
            ...
            package_root / "data" / "anonymized_runtime_data.csv",
        ]
...
def load_static_data(
    csv_path: str = "data/anonymized_static_data.csv",
) -> dict:
    path = Path(csv_path)
    if not path.is_absolute() and not path.exists():
        package_root = Path(__file__).resolve().parent.parent.parent
        candidate_paths = [
            package_root / csv_path,
            Path(__file__).resolve().parent / csv_path,
            package_root / "data" / "anonymized_static_data.csv",
        ]
...
def build_context(runtime_data: pd.DataFrame, static_data: dict | None = None) -> DataContext:
    ...
    callee = sorted(runtime_data["callee"].dropna().unique().tolist())
